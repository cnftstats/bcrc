...
X[, top_cat := ifelse(asset_rank > 1000, "Common cat", "Top 1k cat")]
X[, top_cat := factor(top_cat, levels = c("Common cat", "Top 1k cat"))]
...
X <- X[, .N, .(price_range, market, data_when, top_cat)][order(price_range)]
...
X <- X[, .N, .(price_range, market, sold_time, top_cat)][order(price_range)]
X[price_range %between% c(0, 1999),      x_label := paste0(price_range, "-", price_range + (100-1))]
X[price_range %between% c(2000, 4999),   x_label := paste0(price_range, "-", price_range + (500-1))]
X[price_range %between% c(5000, 9999),   x_label := paste0(price_range, "-", price_range + (1000-1))]
X[price_range %between% c(10000, 29999), x_label := paste0(price_range, "-", price_range + (10000-1))]
...
### Sweep that floor: Lowest top 1k cats listings
```{r}
X <- DT[asset_rank %in% 1:1000][1:15][order(price)]
X[, img := sprintf('<img src="%s%d.png" class="img_html">', img_link, asset_number)]
X[, img := sprintf('<a href="%s" target="_blank">%s</br>#%s</a>', link, img, asset_number)]
...
X[, img := sprintf("https://raw.githubusercontent.com/cnftstats/images/main/bcrc/BCRC%s_large.png", asset_number)]

X[, top_cat := ifelse(asset_rank > 1000, "Common cat", "Top 1k cat")]
X[, top_cat := factor(top_cat, levels = c("Top 1k cat", "Common cat"))]
...
gg <- ggplot(X, aes(asset_rank, price, color = top_cat, customdata = custom_data,
                    text = paste0(asset,
                                  "</br></br>", paste("Price:", round(price, 0),
                                                      "</br>Rank:", asset_rank)))) +
  geom_point(alpha = .6) +
  scale_x_reverse(breaks = scale_x, expand = c(0.03, 0), limits = c(max_x, min_x)) +
  scale_color_manual(values = c("Common cat" = "steelblue3", "Top 1k cat" = "steelblue4")) +
...
gg <- ggplot(X, aes(asset_rank, price, color = top_cat, customdata = custom_data,
  ...
  geom_point(alpha = .6) +
  scale_x_reverse(breaks = scale_x, expand = c(0.03, 0), limits = c(max_x, min_x)) +
  scale_color_manual(values = c("Common cat" = "springgreen3", "Top 1k cat" = "springgreen4")) +
...
X <- X[, .(trait_category, trait, img, price, asset_rank, market, asset_traits_num, asset_traits)]
...
datatable(X, class = 'compact stripe hover',
          options = list(dom = "t",
                         ...
                         info = FALSE,
                         columnDefs = list(list(className = 'dt-right', targets = "_all"))),
          colnames = c("Trait</br>category", "Trait", "Asset", "Price", "Rank", "Market",
...
X <- RAR[, .(asset_name = paste0("Boss Cat Rocket Club #", asset_number),
             asset_rank, asset_traits_num, asset_traits)]
datatable(X, class = 'compact stripe hover',
          options = list(pageLength = 30,
                         ...
                         info = TRUE,
                         columnDefs = list(list(className = 'dt-right', targets = "_all"))),
          colnames = c("Asset", "Rank", "Num.</br>traits", "Traits"),
          rownames = FALSE,
          escape = FALSE) %>%
  formatStyle(columns = 1:ncol(X), fontSize = '90%')
```
